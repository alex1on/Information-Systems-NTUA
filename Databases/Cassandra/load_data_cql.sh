...
table_structure=(
  ...
  "d_date_sk, d_date_id, d_date, d_month_seq, d_week_seq, d_quarter_seq, d_year, d_dow, d_moy, d_dom, d_qoy, d_fy_year, d_fy_quarter_seq, d_fy_week_seq, d_day_name, d_quarter_name,
    d_holiday, d_weekend, d_following_holiday, d_first_dom, d_last_dom, d_same_day_ly, d_same_day_lq, d_current_day, d_current_week, d_current_month, d_current_quarter, d_current_year"
...
eval "cqlsh -u $CASSANDRA_USER -p $CASSANDRA_PASSWORD -e 'SELECT release_version FROM system.local;' $CASSANDRA_HOST > /dev/null 2>&1"
...
copy_data() {
  ...
  COPY_COMMAND="COPY $table_name ($table_structure_cql) FROM '$file' WITH DELIMITER='|' AND HEADER=FALSE"   

  echo "cqlsh -u $CASSANDRA_USER -p $CASSANDRA_PASSWORD -e USE $CASSANDRA_KEYSPACE; $COPY_COMMAND; $CASSANDRA_HOST"

  cqlsh -u $CASSANDRA_USER -p $CASSANDRA_PASSWORD -e "USE $CASSANDRA_KEYSPACE; $COPY_COMMAND;" $CASSANDRA_HOST
