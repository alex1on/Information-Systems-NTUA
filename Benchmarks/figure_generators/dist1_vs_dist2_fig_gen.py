...
distribution1 = pd.read_csv('../query_results_merged/dist_3_workers_cooked.csv')
distribution2 = pd.read_csv('../query_results_merged/dist2_3_workers_bench_14092024.csv')
...
# Filter distribution1 and distribution2 to only include the relevant queries
distribution1_filtered = distribution1[distribution1['query'].isin(queries)].copy()
distribution2_filtered = distribution2[distribution2['query'].isin(queries)].copy()

# Convert time strings to seconds for relevant columns in file2
for col in ['run1', 'run2']:
    distribution1_filtered.loc[:, col] = distribution1_filtered[col].apply(lambda x: time_to_seconds(x) if pd.notna(x) and x != '00:00:00.000' else 0)

# Convert time strings to seconds for relevant columns in file2
for col in ['run1', 'run2']:
    distribution2_filtered.loc[:, col] = distribution2_filtered[col].apply(lambda x: time_to_seconds(x) if pd.notna(x) and x != '00:00:00.000' else 0)
...
# Calculate average times for the Distribution Strategy 1
dist1_strategy_avg_times = calculate_avg(distribution1_filtered, 'run1', 'run2')

# Calculate average times for the Distribution Strategy 1
dist2_strategy_avg_times = calculate_avg(distribution2_filtered, 'run1', 'run2')


# Combine all averages into a single list for plotting
all_avg_times = [
    dist1_strategy_avg_times[0], dist2_strategy_avg_times[0],  # Run 1
    dist1_strategy_avg_times[1], dist2_strategy_avg_times[1],  # Run 2
    dist1_strategy_avg_times[2], dist2_strategy_avg_times[2]   # Combined Avg
]
...
def plot_comparison(avg_times, title, filename):
    ...
    # Labels for x-axis
    x_labels = ['Fact Table ER Based Strategy', 'Optimized Fact and Dimension Consolidation Strategy']
...
plot_comparison(all_avg_times, 'Fact Table ER Based Strategy vs Optimized Strategy', 'dist1_vs_dist2.png')

print("Comparison figure saved as 'dist1_vs_dist2.png'.")
