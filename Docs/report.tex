...
Over the last few years, Trino has draught a lot of attention and nowdays it is widely used by many 
...
In this paper, we benchmark the performnace of Trino across different types of queries, data locations and underlying
...
Relational, Wide-Column and Key-value  provding better results. After, the installation and set up of those three specific stores, we generate and load
data using the well-known \textcolor{linkblue}{\underline{\href{https://www.tpc.org/tpcds/}{TPC-DS}}} benchmark which is also utlized for query generation.
The size of the data is large enough so that it won't fit in memory, in the order of several GB \& milions of records and
the queries are diverse enough including both, simple and complex ones (multiple joins and aggregations).
...
idneitfy how the distributed execution engine and optimization works under a varying ammount of data, data distributed in
...
Trino, formerly known as PrestoDB, is an open-source distributed SQL query engine, build in Java. It is designed to query large ammount of data
...
and other data sources such as Cassandra. Consequently, Trino has the ability to query data lakes housing open column-oriented data file formats 
like ORC or Parquet. Moreover, it excels in executing federated queries that span tables in different data sources like MySQL, PostgreSQL, Cassandra, Redis and MongoDB.
...
Accessing many different data sources, querying data and returning results to client in a distributed way, is a very challenging task. For this purpose Trino utilizes many
different components the most significant and critical of which are connectors, catalogs, schema and table.
...
Trino supports various connectors for traditional SQL databases like MySQL, PostgreSQL, Oracle, for NoSQL databases liek MongoDB and Cassandra and for modern data lakes like 
Hive, Iceberge and Delta Lake. As a result, users can query data from a large variety of different data sources.
...
These features make PostgreSQL a very powerfull and attractive relational DBMS and that's the reason we selected it for our cluster.
...
\begin{itemize}
    \item \textbf{Distirbuted:} Every node in the cluster has the same role and there is no master. Also there is no single point of failure and data is distributed across the cluster, so 
    ...
    \item \textbf{Eventual consistency:} Cassandra is classified as an AP system, meaning that availability and partition tolerance are generaly considered to be more important than consistency. However, 
    Cassandra manages eventual consistency of reads, upserts and deletes through Tombstones.
    \item \textbf{Cassandra Query Language:} Cassandra introduced the Cassandra Query Language (CQL), which is similar to SQL and therefore it can execute complex queries efficiently.
    \item
\end{itemize}
...
data in memoery and because of its design, Redis offer low-latency read and writes, making it particurarly suitable for cases that require a cache. Redis one of the most popular database systems and
...
More in-depth details, instructions and commands can be found in our GitHub repository.
...
In order to allow the communication between the cluster nodes, we set up a local area network (LAN: 192.168.0.0/23). In more details,
192.168.0.2 was assigned to the coordinator, 192.168.1.1 to worker 1 (PostgreSQL), 192.168.1.2 to worker 2 (Cassandra) and 192.168.1.3 to worker 3 (Redis)
...
\begin{itemize}
    ...
    \item \textbf{JVM config:} The JVM config file, \texttt{etc/jvm.config}, contains a list of command line options user for launching the Java Virtual Machine. 
...
\begin{itemize}
    \item \textbf{PostgreSQL:} We installed PostgreSQL 14 on worker 1 (Dowload PostgreSQL \textcolor{linkblue}{\underline{\href{https://www.postgresql.org/download/}{here}}})
...
For Redis, we modified \texttt{/etc/redis/redis.conf} and made Redis listen to worker's 3 IP address by setting bind property that IP address. As previous, we adjusted \texttt{etc/catalog/redis.properties} file.
We should mention here, that Redis doesn't support schema and table definitions. Trino overcomes this issue by forcing the user, to define the schema for each table, using specific table schema definition (JSON in our case)
files. Trino can access these files using their location which is declared in \texttt{redis.properties}.
...
In order to load the data into the different data sources, we created proper database schemas in PostgreSQL and Cassandra (in Redis we utilized table schema definition files as mentioned earlier) through 
...
In conclusion, we also note,that for our initial experiments, we loaded the complete dataset into both Postgres and Cassandra. Following the initial measurements and theoretical insights, we came up with some early coclusions 
which helped up devise a better data distributiob strategy as explained later.
