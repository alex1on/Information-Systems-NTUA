...
Due to the limitations of Redis and the large variety of queries the benchmark provides we chose to select only a part of them and group them as follows:

\begin{itemize}
	\item \textbf{Group 1:} Queries that involve multiple fact tables (e.g., catalog sales and store sales). The actual queries we used in this group are the following: 2, 4, 5, 11, 14, 23, 25, 33, 51, 66, 72, 75, 82, 83, 97.
	\item \textbf{Group 2:} Queries that involve a single fact table. The actual queries we used in this group are the following: 6, 13, 18, 27, 39, 40, 50, 84, 88, 91, 99.
...
The performance measurements from the first test with no data distribution present (\ref{sec:no-data-distribution}) highlighted a clear performance disparity between PostgreSQL, Cassandra and Redis. PostgreSQL was faster in most queries while Redis was significally slower than both of them which was expected due to the poor optimization of Redis on large datasets (add reference). Cassandra while trailing back comparatively to PostgreSQL in most cases, it was faster in some queries while significally slower in some outliers.  %TODO: (insert type of queries that was faster). 
...
We elected the above as our first experiment for two key reasons. First, we aimed to equally partition data throughout the three databases while considering the results of the base test, meaning PostgreSQL would have the higher load while Redis the lowest. Secondly, we opted to replicate any dimension tables that are shared in multiple ERs. Since every fact table is joined with its respective dimension tables we stored every dimension tables wherever was needed in order to maximize data locality. 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{images/DBs_bench_no_distro_alternative/group1.png}
